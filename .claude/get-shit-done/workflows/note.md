...
- **Global scope**: `/home/ubuntu/DnD-proj/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/ubuntu/DnD-proj/.claude/notes/`)
...
2. Glob `/home/ubuntu/DnD-proj/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/ubuntu/DnD-proj/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/ubuntu/DnD-proj/.claude/notes/` — works in any directory
